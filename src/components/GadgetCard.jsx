...
const GadgetCard = ({ data }) => {
  ...
  return (
    ...
    >
      <div className="flex items-center bg-white justify-center p-2 md:py-4 h-48 overflow-hidden">
        <Link to={id && `/gadget/${id}`}>
          <img
            src={img}
            alt={title}
            className="w-36 md:w-36 object-cover duration-200 hover:scale-110"
          />
        </Link>
      </div>
      <div className="flex flex-col justify-evenly gap-5 sm:gap-3 flex-grow w-1/2 md:w-full p-4">
        <Link to={id && `/gadget/${id}`}>
          <h2 className="text-xl capitalize truncate">{title}</h2>
          <p>${price}</p>
        </Link>
